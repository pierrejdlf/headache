...
(function(window, document, navigator) {
  ...
  window.realhumandiff = 0;

  var cameraFound = new CustomEvent(
    "cameraFound", {
      bubbles: true,
      cancelable: true
    }
  ...
  function startStream(stream) {
    ...
    document.dispatchEvent(cameraFound);
    ...
    if(isLittleEndian) {
      for(var y=0; y<move.limitY; ++y) {
        for(var x=0; x<move.limitX; ++x) {
          ...
          var caca = lightnessHasChanged(rawOffset, current);
          if(caca) window.realhumandiff+=1;
          outData[rawOffset] =
              (255 * caca << 24) | //a
